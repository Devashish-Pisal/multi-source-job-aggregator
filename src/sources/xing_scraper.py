...
class XingScraper:
    ...
    def extract_job_urls(self, query_url_list:list[str]) -> tuple[list[dict], list[dict]]:
        ...
        search = xing_scraper_config["search_page"]
        cookie_button = xing_scraper_config["cookie_button"]

        profile_path = Path(scraper_common_config["browser_profile_path"])
        profile_path.mkdir(parents=True, exist_ok=True)
        with sync_playwright() as p:
            logger.info(f"[Xing Scraper] Starting browser session (profile: {profile_path})")
            ...
            try:
                ...
                for index, url in enumerate(query_url_list):
                    try:
                        ...
                        logger.warning(f"[Xing Scraper] Query failed ({consecutive_failures}/{max_failures} consecutive): {error} | URL: {url}")
                        if block_reason or consecutive_failures >= max_failures:
                            ...
                            logger.error(f"[Xing Scraper] Circuit breaker tripped -- {why}. Aborting this scraper with {remaining} of {len(query_url_list)} queries unvisited so a blocking site is not hammered further; {len(accepted_jobs)} accepted / {len(rejected_jobs)} rejected jobs collected so far are kept. Current page URL: {page.url}")
                            break
                    delay_between_queries(scraper_common_config)
                else:
                    logger.info(f"[Xing Scraper] Browser session completed successfully, all {len(query_url_list)} queries visited.")
            finally:
    ...
    def extract_job_descriptions(self, jobs: list[dict], run_timestamp: str) -> list[dict]:
        ...
        profile_path.mkdir(parents=True, exist_ok=True)
        with sync_playwright() as p:
            logger.info(f"[Xing Scraper] Starting browser session for {len(jobs)} job detail pages (profile: {profile_path})")
            context = p.chromium.launch_persistent_context(
                str(profile_path),
                headless=xing_scraper_config["use_headless_mode"], # Headless scraping is not allowed on xing
                channel="chrome",
